...
///     void f(char buf[256]) {
///         memcpy(buf, src, sizeof(buf)); // decayed parameter — same bug
///     }
///
/// `memset` specifically is handled by the existing
/// `bugprone/suspicious-memset` rule, which already catches
/// `memset(_, _, sizeof(any_pointer))` without requiring the sizeof
/// to reference the same variable as the buffer. To avoid
/// double-flagging the canonical `memset(p, 0, sizeof(p))` case, this
/// rule explicitly excludes `memset` from its callee list.
