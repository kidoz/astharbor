...
python-dev:
    cd python && env -u VIRTUAL_ENV uv venv --allow-existing .venv
    cd python && env -u VIRTUAL_ENV uv sync --group dev

# Format the Python package with ruff
python-format:
    cd python && env -u VIRTUAL_ENV uv venv --allow-existing .venv
    cd python && env -u VIRTUAL_ENV uv sync --group dev
    cd python && env -u VIRTUAL_ENV uv run ruff format .

# Lint the Python package with ruff
python-lint:
    cd python && env -u VIRTUAL_ENV uv venv --allow-existing .venv
    cd python && env -u VIRTUAL_ENV uv sync --group dev
    cd python && env -u VIRTUAL_ENV uv run ruff check .

# Type-check the Python package with ty
python-type:
    cd python && env -u VIRTUAL_ENV uv venv --allow-existing .venv
    cd python && env -u VIRTUAL_ENV uv sync --group dev
    cd python && env -u VIRTUAL_ENV uv run ty check

# Run the Python MCP/LSP tests
test-python:
    cd python && env -u VIRTUAL_ENV uv venv --allow-existing .venv
    cd python && env -u VIRTUAL_ENV uv sync --group dev
    cd python && env -u VIRTUAL_ENV uv run pytest -q
